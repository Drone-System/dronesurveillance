...
def is_admin(basestation_id):
    user_id = current_user.id
    data = pd.read_sql("SELECT COUNT(1) FROM users_to_basestations \
                            WHERE user_id=%s::integer AND basestation_id=%s::integer AND owner=TRUE", 
                            conn, params=[user_id, basestation_id])
    has_access = count_bs = data.to_dict()["count"][0] == 1
